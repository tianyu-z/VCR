...
class Lora_Adapter(nn.Module):
    def __init__(self, d_model=None, out_feat=None, r=16, dropout=0.05):
        super().__init__()
...
class VisualAttention(nn.Module):
    ...
    def __init__(
        self, embed_dim, num_heads, bias=True, kdim=None, vdim=None, lora_repeat_num=4
    ):
        ...
        for _ in range(lora_repeat_num):
            self.in_proj_lora.append(
                Lora_Adapter(d_model=embed_dim, out_feat=3 * embed_dim)
            )
        ...
        for _ in range(lora_repeat_num):
            self.out_proj_lora.append(
                Lora_Adapter(d_model=embed_dim, out_feat=embed_dim)
            )
...
class VisualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        x = q_x + self.attention(
            q_x=self.ln_1(q_x), k_x=k_x, v_x=v_x, attn_mask=attn_mask, idx=idx
        )
        residual = x
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, idx=None
    ):
...
class VisionTransformer(nn.Module):
    ...
    def encode(self, image_paths: List[str]):
        ...
        for image_path in image_paths:
            ...
            elif isinstance(image_path, Image_type):
                image = image_path
            else:
