...
class MolmoBlock(nn.Module):
    ...
    def attention(
        ...
        att = self._scaled_dot_product_attention(
            ...
            response_dropout_p=(
                0.0 if not self.training else self.config.response_attention_dropout
            ),
    ...
    def __init__(
        ...
        if v_cfg.attention_dropout > 0:
            self.attention_dropout = Dropout(
                v_cfg.attention_dropout, broadcast_dims=(0, 1)
            )
...
class MultiHeadAttentionPool(nn.Module):
    ...
    def forward(self, inputs_kv: torch.Tensor) -> torch.Tensor:
        ...
        if self.query == "mean":
            ...
            inputs_q = torch.ones_like(inputs_kv[:, :1]) / math.sqrt(
                inputs_kv.shape[-1]
            )
            xq = self.wq(inputs_q)
        else:
...
class Molmo(nn.Module):
    def __init__(self, config: FullMolmoConfig, init_params: bool = True):
        ...
        blocks = [
            MolmoBlock.build(i, config, self.__cache) for i in range(config.n_layers)
        ]
